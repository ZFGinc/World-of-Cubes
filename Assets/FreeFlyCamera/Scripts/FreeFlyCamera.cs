...
public class FreeFlyCamera : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("Select instrument panel")]
    private KeyCode instruments = KeyCode.Tab;
    ...
    [SerializeField]
    [Tooltip("Boost speed")]
    private KeyCode _boostSpeed = KeyCode.LeftControl;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(instruments)) _active = !_active;
        SetCursorState();
        ...
        if (_enableMovement)
        {
            ...
            transform.position = new Vector3(
                Mathf.Clamp(transform.position.x, -25f, 35f),
                Mathf.Clamp(transform.position.y, 3f, 72f),
                Mathf.Clamp(transform.position.z, -25f, 35f)
            );
        }
